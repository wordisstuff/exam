import test from"node:test";import assert from"node:assert/strict";import{accrueTime,createSession,exactMatch,scoreSession,shuffledUnique}from"../lib/engine.ts";import{elapsedSeconds,isExpired,remainingSeconds}from"../lib/time.ts";import{validateQuestionBank}from"../lib/question-validation.ts";import{aggregateMistakes,performance}from"../lib/analytics.ts";import{upsertAttempt}from"../lib/storage.ts";import{questions}from"../data/questions.ts";import type{Attempt,Question,Session}from"../lib/types.ts";
...
test("full exam creates exactly 110 unique questions",()=>{const seed=base(),bank=Array.from({length:111},(_,i)=>({...clone(seed),id:`q${i}`}));const s=createSession("full-exam",bank,999,0,()=>"id");assert.equal(s.questionIds.length,110);assert.equal(new Set(s.questionIds).size,110)});
